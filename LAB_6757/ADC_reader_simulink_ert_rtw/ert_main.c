...
int main(int argc, char **argv)
{
  ...
  rtmSetErrorStatus(ADC_reader_simulink_M, 0);
  ...
  ADC_reader_simulink_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(ADC_reader_simulink_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(ADC_reader_simulink_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(ADC_reader_simulink_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
